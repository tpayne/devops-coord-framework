...
public class BuildTests extends GroovyTestCase {
	...
	/**
	 * Unit tests for printing callbacks
	 */
   	void testDumpFunctions() {
		...
		bld.uploadAssets(body:{println ">Upload assets<"})
      	bld.prepareWorkArea(body:{println ">Prepare workarea<"})
      	String ll = bld.convertCalloutsToJSON()
      	String llx = bld.convertCalloutsToTxt()
      	assertTrue(llx.contains("prepareWorkArea"))
      	assertTrue(ll.contains("runStaticCodeTests"))
		bld.runPipeline()
   	}
