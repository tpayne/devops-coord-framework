...
(java -Duser.home="$JENKINS_HOME" "${java_opts_array[@]}" -jar ${JENKINS_WAR} "${jenkins_opts_array[@]}") &
lpid=$!
echo "Background pid 1 = ${lpid}"
echo "Local pid 1 = $$"

echo "Sleeping..."
sleep 240
echo "Processing plugin..."
cp /var/jenkins_home/devops-framework-plugin.hpi /var/jenkins_home/plugins/devops-framework-plugin.hpi
/usr/local/bin/install-plugins.sh devops-framework-plugin
echo "Kill..."
kill ${lpid}
